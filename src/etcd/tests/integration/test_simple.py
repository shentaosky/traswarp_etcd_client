...
class EtcdIntegrationTest(TestCase):
    cl_size = 3
    ...
    def _get_exe(self):
        ...
        if not program_path:
            raise Exception('docker-compose not in path!!')
...
class TestSimple(EtcdIntegrationTest):
    ...
    def test_machines(self):
        ...
        """
        self.assertEquals(self.client.machines[0].replace(
            'etcd_network', self.ip), self.ipaddr+':6001')

    def test_leader(self):
        """
        INTEGRATION: retrieve leader
        """
        leader_host = self.client.leader['clientURLs'][0].replace(
            'etcd_network', self.ip)
        self.assertIn(leader_host,
            [ self.ipaddr+':6001', self.ipaddr+':6002', self.ipaddr+':6003'])
...
class TestErrors(EtcdIntegrationTest):
    ...
    def test_is_not_a_file(self):
        ...
        self.assertRaises(EtcdNotFile, self.client.set, '/directory',
                          'test-value')
    ...
    def test_creating_already_existing_directory(self):
        ...
        self.assertRaises(EtcdNotFile, self.client.write, '/mydir', None,
                          dir=True)
        self.assertRaises(EtcdAlreadyExist, self.client.write, '/mydir',
                          None, dir=True, prevExist=False)
...
class TestClusterFunctions(EtcdIntegrationTest):
    ...
    def test_reconnect(self):
        ...
        """
        self.client = Client(host=self.ip, port=6001, allow_reconnect=True)
    ...
    def test_reconnet_fails(self):
        ...
        self.processHelper.kill_one(0)
        self.assertRaises(EtcdException, self.client.get, '/test_set')
...
class TestWatch(EtcdIntegrationTest):
    ...
    def test_watch(self):
        ...
        def change_value(key, newValue):
            c = Client(host=self.ip, port=6001)
            c.set(key, newValue)
